...
describe('Version', () => {
  describe('parseVersion', () => {
    ...
    it('parses a correct 3-point version with preRelease', () => {
      fc.assert(fc.property(fc.integer(0, 200), fc.integer(0, 200), fc.integer(0, 100), fc.hexaString(1, 100), (major, minor, patch, preRelease) => {
        const input = `${major}.${minor}.${patch}-${preRelease}`;
        const actual = Version.parseVersion(input);
        const expected = E.right({ major, minor, patch, preRelease, buildMetaData: undefined });
        assert.deepEqual(actual, expected);
      }));
    });

    it('parses a correct 3-point version with buildmeta', () => {
      fc.assert(fc.property(fc.integer(0, 200), fc.integer(0, 200), fc.integer(0, 100), fc.hexaString(1, 100), (major, minor, patch, buildMetaData) => {
        const input = `${major}.${minor}.${patch}+${buildMetaData}`;
        const actual = Version.parseVersion(input);
        const expected = E.right({ major, minor, patch, preRelease: undefined, buildMetaData });
        assert.deepEqual(actual, expected);
      }));
    });
  });
