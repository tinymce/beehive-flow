...
export const parseArgs = async (args: string[]): Promise<BeehiveArgs> => {
  ...
  if (cmd === 'prepare') {
    return BeehiveArgs.prepareArgs(dryRun, workingDir, temp, gitUrl);

  } else if (cmd === 'release') {
    const mm = await parseMajorMinorVersion(a.majorDotMinor as string);
    return BeehiveArgs.releaseArgs(dryRun, workingDir, temp, gitUrl, mm);

  } else if (cmd === 'advance') {
    const mm = await parseMajorMinorVersion(a.majorDotMinor as string);
    return BeehiveArgs.advanceArgs(dryRun, workingDir, temp, gitUrl, mm);

  } else if (cmd === 'advance-ci') {
    return BeehiveArgs.advanceCiArgs(dryRun, workingDir);

  } else if (cmd === 'stamp') {
    return BeehiveArgs.stampArgs(dryRun, workingDir);

  } else if (cmd === 'publish') {
    return BeehiveArgs.publishArgs(dryRun, workingDir);
